...
def calculate_properties_uv(z: List[float], u: float, v: float, units_system: str = 'SI') -> Dict[str, Any]:
    """
    Calculate fluid properties at given internal energy and specific volume with specified unit system.
    ...
    # Use DEFLSHdll (Density-Energy) for UV flash
    T, P, Dl, Dv, x, y, q, h, s, Cv, Cp, w, ierr, herr = RP.DEFLSHdll(D, u, z)
    if ierr > 0:
        raise ValueError(f"Error in DEFLSHdll: {herr}")
...
def uv_flash():
    try:
        ...
        if 'internal_energy' not in variables or 'specific_volume' not in variables:
            return jsonify({'error': 'Missing internal_energy or specific_volume variables'}), 400
        ...
        try:
            ...
            u_from = float(energy_range.get('from', 0.0))
            u_to = float(energy_range.get('to', 10000.0))
            v_from = float(volume_range.get('from', 0.01))
            v_to = float(volume_range.get('to', 1.0))
            
            energy_resolution = float(variables['internal_energy'].get('resolution', 500.0))
            volume_resolution = float(variables['specific_volume'].get('resolution', 0.05))
            
            # Ensure to > from
            if u_to <= u_from:
                u_to = u_from + energy_resolution
            if v_to <= v_from:
                v_to = v_from + volume_resolution
                
            # Update the ranges for later use
            energy_range = {'from': u_from, 'to': u_to}
        ...
        # Debug log
        print(f"Calculating UV flash for internal energy range: {energy_range['from']} to {energy_range['to']} J/mol, "
        ...
        if grid_type.lower() != 'equidistant':
            ...
            if grid_type.lower() == 'adaptive':
                try:
                    # Get phase boundaries in U-V space
                    u_boundaries, v_boundaries = get_phase_boundaries_uv(
                        RP, z, energy_range, volume_range
                    )
                    
                    print(f"Identified phase boundaries: {len(u_boundaries)} energy points, "
            ...
            print(f"Generated {len(u_range)} internal energy points and {len(v_range)} specific volume points")
            
        else:
            # Create regular (equidistant) grids as before
            u_range = np.arange(
                u_from,
                u_to + energy_resolution,
                energy_resolution
            )
        ...
        for u_idx, u in enumerate(u_range):
            for v_idx, v in enumerate(v_range):
                try:
                    props = calculate_properties_uv(z, float(u), float(v), units_system)
                    filtered_props = {k: v for k, v in props.items() 
                                   if k in properties or k in ['temperature', 'pressure', 'internal_energy', 'specific_volume', 'phase']}
                    ...
                    print(f"Error processing u={u} J/mol, v={v} m³/mol: {fe}", file=sys.stderr)
                    continue
        ...
        if response_format.lower() == 'olga_tab':
            ...
            try:
                ...
                energy_vars = {
                    'range': {'from': u_range.min(), 'to': u_range.max()},
                    'resolution': energy_resolution,
                    'values': u_range  # Pass the actual grid values
                }
                ...
                response = format_olga_tab(
                    energy_vars,       # x-axis (internal energy)
            ...
            return jsonify({
                ...
                'grid_info': {
                    'type': grid_type,
                    'internal_energy_points': len(u_range),
